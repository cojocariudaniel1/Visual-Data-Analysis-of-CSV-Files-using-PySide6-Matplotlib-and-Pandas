...
def harta_vanzari_pe_an():
    # Exemplu de DataFrame
    dfx = get_data()

    df = add_state_codes(dfx)
    # Adaugă coloana cu codurile statelor
    result_df = df.groupby(["State Code", pd.to_datetime(df["Order Date"]).dt.to_period("Y")])["Sales"].sum().reset_index()

    fig = px.choropleth(result_df, locations='State Code', color='Sales', hover_name='State Code', locationmode='USA-states',
                        title='Sales per State in USA States', scope="usa", animation_frame="Order Date")
    fig.show()
